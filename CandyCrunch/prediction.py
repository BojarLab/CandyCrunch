...
mass_dict = dict(
    zip(mapping_file.composition, mapping_file["underivatized_monoisotopic"])
)
...
def process_mzML_stack(filepath, num_peaks=1000, ms_level=2, intensity=False):
    ...
    for spectrum in run:
        if spectrum.ms_level == ms_level:
            try:
                ...
                continue
            mz_i_dict = {}
            for mz, i in spectrum.highest_peaks(
                min(num_peaks, len(spectrum.peaks("raw")))
            ):
                mz_i_dict[mz] = i
            if mz_i_dict:
    ...
    with mzxml.read(filepath) as reader:
        for spectrum in reader:
            if spectrum["msLevel"] == ms_level:
                ...
                for mz, i in zip(
                    spectrum["m/z array"][:num_peaks_to_extract],
                    spectrum["intensity array"][:num_peaks_to_extract],
                ):
                    mz_i_dict[mz] = i
                if mz_i_dict:
                    key = (
                        f"{spectrum['id']}_{spectrum['precursorMz'][0]['precursorMz']}"
                    )
                    highest_i_dict[key] = mz_i_dict
                    reducing_mass.append(float(key.split("_")[-1]))
                    rts.append(spectrum["retentionTime"])
                    if intensity:
                        inty = spectrum["precursorMz"][0].get(
                            "precursorIntensity", np.nan
                        )
                        intensities.append(inty)
...
def bin_intensities(peak_d, frames):
    ...
    for b, s, m in zip(
        unq,
        np.bincount(ids, np.array(list(map(peak_d.get, keys)))),
        np.bincount(range(len(a2)), a2),
    ):
        out_list[b - 1] = s
        out_list2[b - 1] = m
...
def process_for_inference(
    ...
    df["binned_intensities"], df["mz_remainder"] = list(
        zip(*[bin_intensities(k, frames) for k in df.peak_d])
    )
    # Dataloader generation
    X = list(
        zip(
            df.binned_intensities.values.tolist(),
            df.mz_remainder.values.tolist(),
            df.reducing_mass.values.tolist(),
            df.glycan_type.values.tolist(),
            df.RT2.values.tolist(),
            df["mode"].values.tolist(),
            df.lc.values.tolist(),
            df.modification.values.tolist(),
            df.trap.values.tolist(),
        )
    )
    ...
    dset = SimpleDataset(
        X,
        y,
        transform_mz=transform_mz,
        transform_prec=transform_prec,
...
def get_rep_spectra(rt_label_in, intensity=False):
    ...
    medIdx = [
        rt_label_in.index(tt[k][i])
        if isinstance(tt[k][i], float)
        else rt_label_in.index(tt[k][i][0])
        for k, i in enumerate(medIdx)
    ]
    if intensity:
...
def build_mean_dic(dicty, rt, intensity):
    ...
    rt_idx, inty_idx, num_spectra = list(
        zip(*[get_rep_spectra(k, intensity=True) for k in rt_labels])
    )
    num_spectra = unwrap(num_spectra)
    if inty_check:
        intensity = [intensity[k] for k in sort_idx]
        intensity = [
            [intensity[k] for k in range(len(intensity)) if labels[k] == j]
            for j in unq_labels
        ]
        intensity = [
            [
                [intensity[k][z] for z in j]
        ...
        intensity = unwrap(
            [[sum(j) for j in k] if isinstance(k[0], list) else [k] for k in intensity]
        )
    # Get m/z, predictions, and prediction confidence of those representative spectra
    keys = [
        [list(dicty.keys())[k] for k in range(len(labels)) if labels[k] == j]
        for j in unq_labels
    ]
    keys = [
        [
            [keys[k][z] for z in j] if isinstance(j, list) else [keys[k][j]]
            for j in inty_idx[k]
        ]
        for k in range(len(keys))
    ]
    keys = unwrap(
        [[np.mean(j) for j in k] if isinstance(k[0], list) else [k] for k in keys]
    )
    values = [
        [list(dicty.values())[k] for k in range(len(labels)) if labels[k] == j]
        for j in unq_labels
    ]
    rep_values = unwrap([[values[k][j] for j in rt_idx[k]] for k in range(len(values))])
    values = [
        [
            [values[k][z] for z in j] if isinstance(j, list) else [values[k][j]]
            for j in inty_idx[k]
        ]
    ...
    values = unwrap(
        [
            [average_dicts(j) for j in k] if isinstance(k[0], list) else [k]
            for k in values
        ]
    )
    rts = [
        [
            [rt_labels[k][z] for z in j] if isinstance(j, list) else [rt_labels[k][j]]
            for j in inty_idx[k]
        ]
        for k in range(len(rt_labels))
    ]
    rts = unwrap(
        [[np.mean(j) for j in k] if isinstance(k[0], list) else [k] for k in rts]
    )
    if inty_check:
...
def deduplicate_predictions(df):
    ...
    if "rel_abundance" in df.columns:
        struc_abundance = {
            (
                df.predictions.values.tolist()[k][0][0]
                if len(df.predictions.values.tolist()[k]) > 0
                else "ID" + str(df.index.tolist()[k])
            ): df.rel_abundance.values.tolist()[k]
    ...
    for k in range(len(df) - 1):
        ...
        if k not in drop_idx and len(df.predictions.values.tolist()[k]) > 0:
            check = [
                abs(df.index.tolist()[k] - df.index.tolist()[j])
                < determine_threshold(df.index.tolist()[k]) + 0.5
                and df.predictions.values.tolist()[k][0][0]
                == df.predictions.values.tolist()[j][0][0]
                if len(df.predictions.values.tolist()[j]) > 0
                else False
            ...
            if any(check):
                ...
                winner = idx[
                    np.argmax([df.predictions.values.tolist()[i][0][1] for i in idx])
                ]
                    ...
                    if "rel_abundance" in df.columns:
                        if len(df.predictions.values.tolist()[k]) > 0:
                            struc_abundance[
                                ...
                                [
                                    df.iat[
                                        i, df.columns.tolist().index("rel_abundance")
                                    ]
                                    for i in idx
                                ]
                            )
    ...
    if "rel_abundance" in df.columns:
        df.rel_abundance = [
            struc_abundance[df.predictions.values.tolist()[k][0][0]]
            if len(df.predictions.values.tolist()[k]) > 0
            else struc_abundance["ID" + str(df.index.tolist()[k])]
            for k in range(len(df))
        ]
    return df
...
def combinatorics(comp):
    ...
    all_combinations = set(
        unwrap(
            [
                [comb for comb in combinations(verbose_clist, i)]
...
def domain_filter(
    ...
    for k in range(len(df_out)):
        ...
        if len(df_out.predictions.values.tolist()[k]) > 0:
            ...
            current_preds = [
                "".join(list(df_out.composition.values.tolist()[k].keys()))
            ]
            to_append = False
        # Check whether it's a glycan spectrum
        if not np.any(
            np.abs(
                np.array(df_out.top_fragments.values.tolist()[k][:10])[:, None]
                - cmasses
            )
            < 1.5
        ):
        ...
        for i, m in enumerate(current_preds):
            ...
            if "Neu5Ac" in m:
                truth.append(
                    any(
                        [
                            abs(mass_dict["Neu5Ac"] + (1.0078 * multiplier) - j) < 1
                            or abs(assumed_mass - mass_dict["Neu5Ac"] - j) < 1
                            or abs(
                                df_out.index.tolist()[k]
                                - ((mass_dict["Neu5Ac"] - addy) / c)
                                - j
                            )
                            < 1
                            for j in df_out.top_fragments.values.tolist()[k]
            ...
            if "Neu5Gc" in m:
                truth.append(
                    any(
                        [
                            abs(mass_dict["Neu5Gc"] + (1.0078 * multiplier) - j) < 1
                            or abs(assumed_mass - mass_dict["Neu5Gc"] - j) < 1
                            or abs(
                                df_out.index.tolist()[k]
                                - ((mass_dict["Neu5Gc"] - addy) / c)
                                - j
                            )
                            < 1
                            for j in df_out.top_fragments.values.tolist()[k]
            ...
            if "Kdn" in m:
                truth.append(
                    any(
                        [
                            abs(mass_dict["Kdn"] + (1.0078 * multiplier) - j) < 1
                            or abs(assumed_mass - mass_dict["Kdn"] - j) < 1
                            or abs(
                                df_out.index.tolist()[k]
                                - ((mass_dict["Kdn"] - addy) / c)
                                - j
                            )
                            < 1
                            for j in df_out.top_fragments.values.tolist()[k]
            ...
            if "Neu5Gc" not in m:
                truth.append(
                    not any(
                        [
                            abs(mass_dict["Neu5Gc"] + (1.0078 * multiplier) - j) < 0.5
                            for j in df_out.top_fragments.values.tolist()[k][:5]
                            if isinstance(j, float)
                        ]
                    )
                )
            if "Neu5Ac" not in m and "Neu5Gc" not in m:
                truth.append(
                    not any(
                        [
                            abs(mass_dict["Neu5Ac"] + (1.0078 * multiplier) - j) < 0.5
                            for j in df_out.top_fragments.values.tolist()[k][:5]
                            if isinstance(j, float)
                        ]
                    )
            ...
            if "Neu5Ac" not in m and (m.count("Fuc") + m.count("dHex") > 1):
                truth.append(
                    not any(
                        [
                            abs(mass_dict["Neu5Ac"] + (1.0078 * multiplier) - j) < 1
                            or abs(df_out.index.tolist()[k] - mass_dict["Neu5Ac"] - j)
                            < 1
                            for j in df_out.top_fragments.values.tolist()[k][:10]
                            if isinstance(j, float)
                        ]
                    )
                )
                ...
                truth.append(
                    any(
                        [
                            ...
                            for t in df_out.top_fragments.values.tolist()[k][:20]
                        ]
                    )
                )
            ...
            if c > 1:
                truth.append(
                    any(
                        [
                            j > df_out.index.values[k] * 1.2
                            for j in df_out.top_fragments.values.tolist()[k][:15]
                        ]
                    )
                )
            if c == 1:
                ...
                truth.append(
                    any(
                        [
                            abs(
                                df_out.index.tolist()[k]
                                - mass_dict[df_out.adduct.values.tolist()[k]]
                                - j
                            )
                            < 0.5
                            for j in df_out.top_fragments.values.tolist()[k][:10]
    ...
    idx = [
        k
        for k in range(len(df_out))
        if "remove" not in df_out.predictions.values.tolist()[k][:1]
    ]
...
def backfill_missing(df):
    ...
    for k in range(len(df)):
        if not len(df.predictions.values.tolist()[k]) > 0:
            idx = [
                j
                for j in range(len(str_dics))
                if str_dics[j] == str_dics[k]
                and abs(
                    df.index.tolist()[j] * abs(df.charge.values.tolist()[j])
                    + (abs(df.charge.values.tolist()[j]) - 1)
                    - df.index.tolist()[k] * abs(df.charge.values.tolist()[k])
                    + (abs(df.charge.values.tolist()[k]) - 1)
                )
                < determine_threshold(df.index.tolist()[j])
...
def adduct_detect(df, mode, modification):
    ...
    for k in range(len(df)):
        if (
            abs(
                composition_to_mass(df.composition.values.tolist()[k])
                + adduct_mass
                - (
                    df.index.tolist()[k] * abs(df.charge.values.tolist()[k])
                    + (abs(df.charge.values.tolist()[k]) - 1)
...
def average_preds(preds, conf, k=5):
    ...
    for c in range(len(pred_chunks)):
        ...
        combs = [
            {this_pred[j][m]: this_conf[j][m] for m in range(len(this_pred[j]))}
...
def impute(df_out):
    ...
    for k in range(len(df_out)):
        for j in range(len(df_out)):
            if (
                len(df_out.predictions.values.tolist()[k]) > 0
                and len(df_out.predictions.values.tolist()[j]) < 1
            ):
                if (
                    abs(
                        df_out.index.tolist()[k]
                        + (16.0051 / max([abs(df_out.charge.values.tolist()[k]), 1]))
                        - df_out.index.tolist()[j]
                    )
                    < 0.5
                    and "Neu5Gc" in df_out.composition.values.tolist()[j].keys()
                ):
                    df_out.iat[j, 0] = [
                        (m[0].replace("Neu5Ac", "Neu5Gc", 1),)
                        for m in df_out.predictions.values.tolist()[k]
                    ]
                elif (
                    abs(
                        df_out.index.tolist()[k]
                    ...
                    < 0.5
                    and "Neu5Ac" in df_out.composition.values.tolist()[j].keys()
                ):
                    df_out.iat[j, 0] = [
                        (m[0].replace("Neu5Gc", "Neu5Ac", 1),)
                        for m in df_out.predictions.values.tolist()[k]
                    ]
    return df_out
...
def make_mass_dic(glycans, glycan_class, filter_out, df_use, taxonomy_class="Mammalia"):
    ...
    mass_dic = {
        u: [exp_glycans[j] for j in [i for i, m in enumerate(masses) if m == u]]
        for u in unq_masses
    }
    return mass_dic
...
def canonicalize_biosynthesis(df_out, libr, pred_thresh):
    ...
    df_out["true_mass"] = [
        df_out.index.tolist()[j] * abs(charges[j]) - (charges[j] + modifier)
        for j in range(len(df_out))
    ]
    ...
    for k in reversed(range(len(df_out))):
        ...
        if len(preds) > 0:
            rest_top1 = set(
                [
                    df_out.predictions.values.tolist()[j][0][0]
                    for j in range(k)
                    if len(df_out.predictions.values.tolist()[j]) > 0
                    and df_out.evidence.values.tolist()[j] == "strong"
                ]
            )
            for i, p in enumerate(preds):
                ...
                p[1] += 0.1 * sum(
                    [
                        subgraph_isomorphism(p[0], t, libr=libr, wildcards_ptm=True)
                        * (not t == p[0])
                        for t in rest_top1
                    ]
                )
...
def load_spectra_filepath(spectra_filepath):
    if spectra_filepath.endswith(".xlsx"):
        ...
        loaded_file = loaded_file.iloc[
            [
                k
                for k in range(len(loaded_file))
                if loaded_file.peak_d.values.tolist()[k][-1] == "}"
                and loaded_file.RT[k] > 2
            ],
            :,
        ]
        loaded_file.peak_d = [
            ast.literal_eval(k) if k[-1] == "}" else np.nan for k in loaded_file.peak_d
        ]
...
def wrap_inference(
    ...
    mass_tolerance=0.5,
    extra_thresh=0.2,
    filter_out={"Kdn", "P", "HexA", "Pen", "HexN", "Me", "PCho", "PEtN"},
    supplement=True,
    experimental=True,
    mass_dic=None,
    ...
    print(
        ...
        + " LC, and "
        + trap
        + " ion trap. If any of that seems off to you, please restart with correct parameters."
    )
    if libr is None:
        libr = lib
    if df_use is None:
        df_use = copy.deepcopy(
            df_glycan[
                (df_glycan.glycan_type == glycan_class)
                & (df_glycan.Class.str.contains(taxonomy_class))
            ]
        )
    ...
    if intensity:
        intensity = (
            False
            if (loaded_file["intensity"] == 0).all()
            or loaded_file["intensity"].isnull().all()
            else True
        )
    # Prepare file for processing
    loaded_file.dropna(subset=["peak_d"], inplace=True)
    loaded_file.reducing_mass = [
        k + np.random.uniform(0.00001, 10 ** (-20)) for k in loaded_file.reducing_mass
    ]
    ...
    coded_class = (
        ...
        if any([glycan_class == "free", glycan_class == "lipid"])
        else 3
    )
    spec_dic = {
        loaded_file.reducing_mass.values.tolist()[
            k
        ]: loaded_file.peak_d.values.tolist()[k]
        for k in range(len(loaded_file))
    }
    # Group spectra by mass/retention isomers and process them for being inputs to CandyCrunch
    keys, values, RT, num_spectra, rep_values, intensity = build_mean_dic(
        spec_dic, loaded_file.RT.values.tolist(), inty
    )
    loader, df_out = process_for_inference(
        keys,
        values,
        RT,
        num_spectra,
    ...
    preds, pred_conf = get_topk(
        loader, model, glycans, temp=True, temperature=temperature
    )
    preds, pred_conf = average_preds(preds, pred_conf)
    if intensity:
        df_out["rel_abundance"] = intensity
    df_out["predictions"] = [
        [(preds[k][j], pred_conf[k][j]) for j in range(len(preds[k]))]
    ...
    df_out.predictions = [
        [
            gly
            for gly in v
            if enforce_class(gly[0], glycan_class, gly[1], extra_thresh=extra_thresh)
            and gly[1] > pred_thresh
        ]
        for v in df_out.predictions
    ]
    df_out.predictions = [
        [
            (gly[0], round(gly[1], 4))
            for gly in df_out.predictions.values.tolist()[v]
            if mass_check(
                df_out.index.tolist()[v],
                gly[0],
                libr=libr,
                modification=modification,
                mode=mode,
        ...
        for v in range(len(df_out))
    ]
    # Get composition of predictions
    df_out["composition"] = [
        glycan_to_composition(g[0][0]) if len(g) > 0 and len(g[0]) > 0 else np.nan
        for g in df_out.predictions
    ]
    df_out.composition = [
        k
        if isinstance(k, dict)
        else mz_to_composition(
            df_out.index.tolist()[i],
            mode=mode,
            mass_tolerance=mass_tolerance,
            glycan_class=glycan_class,
            df_use=df_use,
        ...
        for i, k in enumerate(df_out.composition.values.tolist())
    ]
    df_out.composition = [
        np.nan
        ...
        for k in df_out.composition
    ]
    df_out.dropna(subset=["composition"], inplace=True)
    # Calculate precursor ion charge
    df_out["charge"] = [
        round(
            composition_to_mass(df_out.composition.values.tolist()[k])
            / df_out.index.values.tolist()[k]
        )
        * multiplier
        for k in range(len(df_out))
    ]
    ...
    df_out["evidence"] = [
        "strong" if len(k) > 0 else np.nan for k in df_out.predictions
    ]
    ...
    if supplement:
        try:
            df_out = supplement_prediction(
                df_out, glycan_class, libr=libr, mode=mode, modification=modification
            )
            df_out.evidence = [
                "medium"
                if isinstance(df_out.evidence.values.tolist()[k], float)
                and len(df_out.predictions.values.tolist()[k]) > 0
                else df_out.evidence.values.tolist()[k]
                for k in range(len(df_out))
            ]
        except:
            pass
    ...
    if experimental:
        ...
        if mass_dic is None:
            mass_dic = make_mass_dic(
                glycans, glycan_class, filter_out, df_use, taxonomy_class=taxonomy_class
            )
    ...
    if supplement or experimental:
        ...
        df_out.predictions = [
            [
                (k[0].replace("-ol", "").replace("1Cer", ""), k[1])
                if len(k) > 1
                else (k[0].replace("-ol", "").replace("1Cer", ""),)
                for k in j
            ]
            if len(j) > 0
            else j
            for j in df_out.predictions
        ]
    ...
    # Clean-up
    df_out.composition = [
        glycan_to_composition(k[0][0])
        if len(k) > 0
        else df_out.composition.values.tolist()[i]
        for i, k in enumerate(df_out.predictions)
    ]
    df_out.charge = [
        round(
            composition_to_mass(df_out.composition.values.tolist()[k])
    ...
    if intensity:
        df_out.rel_abundance = [
            k / sum(df_out.rel_abundance.values.tolist()) * 100
            for k in df_out.rel_abundance
        ]
    if spectra:
...
def supplement_prediction(
    df_in, glycan_class, libr=None, mode="negative", modification="reduced"
):
    ...
    unexplained_idx = [
        k for k in range(len(df)) if len(df.predictions.values.tolist()[k]) < 1
    ]
    ...
    explained_idx = [
        np.where(
            [
                mass_check(j, k, modification=modification, mode=mode)
    ...
    new_nodes = [
        (new_nodes[k], explained_idx[k])
        for k in range(len(new_nodes))
        if len(explained_idx[k]) > 0
    ]
...
def calculate_df_vectors(df_out_list: list, labels: list, strong_only=True):
    df_vectors = []
    for df_out in df_out_list:
        temp = df_out
        if strong_only:
            temp = df_out[df_out.evidence == "strong"]
        abundances = [
            k / sum(temp.num_spectra.values.tolist()) * 100 for k in temp.num_spectra
        ]
        top1_predictions = [x[0][0] for x in temp.predictions]
        temp = pd.DataFrame(abundances).T
        temp.columns = top1_predictions
        temp = temp.groupby(axis=1, level=0).sum()
        if len(temp) < 1:
            temp = pd.Series(np.nan)
        df_vectors.append(temp)
    out = pd.concat(df_vectors, axis=0, ignore_index=True)
    out = out.T
    out.columns = labels
    out = out.reset_index()
    return out
...
def prepare_input_files(file_list: list, glycan_class, mode, modification, lc, trap):
    outputs = []
    for x in file_list:
        if ".csv" in x:
            df_out = pd.read_csv(x)
            df_out["predictions"] = [ast.literal_eval(x) for x in df_out.predictions]
            outputs.append(df_out)
        elif any([ext in x for ext in [".mzML", "mzXML", ".xlsx"]]):
...
def DE_pipeline(
    control_filenames: list,
    treatment_filenames: list,
    glycan_class,
    strong_only=True,
    mode="negative",
    modification="reduced",
    ...
    control_labels = [x.split("/")[-1].split(".")[0] for x in control_filenames]
    treatment_labels = [x.split("/")[-1].split(".")[0] for x in treatment_filenames]
    outputs = prepare_input_files(
        control_filenames + treatment_filenames,
        glycan_class=glycan_class,
        mode=mode,
        modification=modification,
        lc=lc,
    ...
    df_vectors = calculate_df_vectors(
        outputs, control_labels + treatment_labels, strong_only=strong_only
    )
    de_res = get_differential_expression(
        df_vectors, control_labels, treatment_labels, paired=True
    )
    return de_res
