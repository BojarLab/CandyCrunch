        ...
        for spectrum in reader:
            if spectrum["msLevel"] == ms_level:
                ...
                mz_i_dict = {
                    mz: i
                    for mz, i in zip(
                        mz_array[:num_peaks_to_extract],
                        intensity_array[:num_peaks_to_extract],
                    )
                }
                if mz_i_dict:
...
def average_dicts(dicts, mode="mean"):
    ...
    return {
        mass: np.mean(intensities) if mode == "mean" else max(intensities)
...
def process_for_inference(
    df, glycan_class, mode="negative", modification="reduced", lc="PGC", trap="linear"
):
    ...
    df = df.assign(
        ...
        modification=np.select(
            [modification == "reduced", modification == "permethylated"], [0, 1], 2
        ),
        trap=np.select(
            [trap == "linear", trap == "orbitrap", trap == "amazon"], [0, 1, 2], 3
        ),
    )
    df["glycan"] = [0] * len(df)
...
def mass_check(
    ...
    try:
        mz = glycan_to_mass(
            glycan,
            sample_prep=modification
            if modification in ["permethylated", "peracetylated"]
            else "underivatized",
        )
    except:
        return False
    mz += (
        modification_mass_dict[modification]
        if modification in modification_mass_dict
        else mass_tag
    )
    adduct_list = (
        ["Acetonitrile", "Acetate", "Formate"]
        if mode == "negative"
        else ["Na+", "K+", "NH4+"]
    )
...
def condense_dataframe(
    df, mz_diff=0.5, rt_diff=1.0, min_mz=39.714, max_mz=3000, bin_num=2048
):
    ...
    for _, row in df.iloc[1:].iterrows():
        ...
        for cluster in clusters:
            ...
            if abs(last_rm - rm) <= mz_diff and abs(last_rt - rt) <= rt_diff:
                ...
                cluster["max_intensity"].append(
                    intensity if intensity > last_max else last_max
                )
    ...
    for cluster in clusters:
        ...
        binned_intensities, mz_remainder = zip(
            *[bin_intensities(c, frames) for c in cluster["peak_d"]]
        )
...
def deduplicate_predictions(df, mz_diff=0.5, rt_diff=1.0):
    ...
    for idx, row in df.iterrows():
        ...
        mask = (np.abs(df.index - idx) < mz_diff) & (
            np.abs(df["RT"] - row["RT"]) < rt_diff
        )
        ...
        pred_mask = (
            sub_df["predictions"].apply(lambda x: x[0][0] if x else None) == first_pred
        )
        # Choose the row with the max confidence for this prediction
        max_conf_row = sub_df.loc[pred_mask].iloc[
            np.argmax([p[0][1] for p in sub_df.loc[pred_mask, "predictions"]])
        ]
...
def domain_filter(
    ...
    for k in range(len(df_out)):
        ...
        current_preds = (
            df_out["predictions"].iloc[k]
            if len(df_out["predictions"].iloc[k]) > 0
            else ["".join(list(df_out["composition"].iloc[k].keys()))]
        )
...
def adduct_detect(df, mode, modification):
    ...
    computed_masses = np.array(
        [composition_to_mass(composition) for composition in compositions]
    )
...
def average_preds(preds, conf, k=5):
    ...
    for this_pred, this_conf in zip(pred_chunks, conf_chunks):
        combs = [
            {pred: conf for pred, conf in zip(chunk_pred, chunk_conf)}
            for chunk_pred, chunk_conf in zip(this_pred, this_conf)
        ]
...
def generate_variants(sequence):
    ...
    occurrences = [
        (m.start(), m.group()) for m in re.finditer(r"Neu5(Ac|Gc)", sequence)
    ]
...
def impute(df_out, libr=None, mode="negative", modification="reduced", mass_tag=0):
    ...
    seqs = [
        ...
        if p and ("Neu5Ac" in p[0][0] or "Neu5Gc" in p[0][0])
    ]
...
def canonicalize_biosynthesis(df_out, libr, pred_thresh):
    ...
    """
    df_out["true_mass"] = df_out.index * abs(df_out["charge"]) - (
        df_out["charge"] + np.sign(df_out["charge"].sum())
    )
    df_out.sort_values(by="true_mass", inplace=True)
    strong_evidence_preds = [
        pred[0][0]
        for pred, evidence in zip(df_out["predictions"], df_out["evidence"])
        if len(pred) > 0 and evidence == "strong"
    ]
    ...
    for k, row in df_out[::-1].iterrows():
        ...
        for p in preds:
            ...
            if len(p_list) == 1:
                p_list.append(0)
            p_list[1] += 0.1 * sum(
                subgraph_isomorphism(p_list[0], t, libr=libr, wildcards_ptm=True)
                for t in rest_top1
                if t != p_list[0]
            )
...
def combine_charge_states(df_out):
    ...
    for pred in filtered_top_pred:
        idx = df_filtered.index[
            len(df_filtered)
            - 1
            - df_filtered.top_pred.values.tolist()[::-1].index(pred)
        ]
    ...
    df_out = pd.concat(
        [df_out[~df_out["top_pred"].isin(filtered_top_pred)], df_filtered]
    ).sort_index()
...
def wrap_inference(
    ...
    if rt_max:
        ...
        loaded_file = loaded_file[
            loaded_file["RT"] < 0.9 * loaded_file["RT"].max()
        ].reset_index(drop=True)
    intensity = (
        "intensity" in loaded_file.columns
        and not (loaded_file["intensity"] == 0).all()
        and not loaded_file["intensity"].isnull().all()
    )
    if intensity:
    ...
    loaded_file["reducing_mass"] += np.random.uniform(
        0.00001, 10 ** (-20), size=len(loaded_file)
    )
    coded_class = {"O": 0, "N": 1, "free": 2, "lipid": 2}[glycan_class]
    spec_dic = {
        mass: peak
        for mass, peak in zip(
            loaded_file["reducing_mass"].values, loaded_file["peak_d"].values
        )
    }
    # Group spectra by mass/retention isomers and process them for being inputs to CandyCrunch
    df_out = condense_dataframe(
        loaded_file, mz_diff=mass_tolerance, rt_diff=rt_diff, bin_num=bin_num
    )
    loader, df_out = process_for_inference(
        df_out, coded_class, mode=mode, modification=modification, lc=lc, trap=trap
    )
    ...
    df_out["predictions"] = [
        [(pred, conf) for pred, conf in zip(pred_row, conf_row)]
    ...
    df_out["predictions"] = [
        [
            (g[0], round(g[1], 4))
            for g in preds
            if enforce_class(g[0], glycan_class, g[1], extra_thresh=extra_thresh)
            and g[1] > pred_thresh
            and mass_check(
                mass,
                g[0],
    ...
    df_out["composition"] = [
        glycan_to_composition(g[0][0]) if g and g[0] else np.nan
    ...
    df_out.composition = [
        np.nan
        if isinstance(k, list) and not k
        else k[0]
    ...
    df_out["top_fragments"] = [
        [
            round(frag[0], 4)
            for frag in sorted(peak_d.items(), key=lambda x: x[1], reverse=True)[
                :frag_num
            ]
        ]
    ...
    df_out["evidence"] = [
        "strong" if preds else np.nan for preds in df_out["predictions"]
    ]
    ...
    if experimental:
        df_out = impute(
            df_out, libr=libr, mode=mode, modification=modification, mass_tag=mass_tag
        )
        mass_dic = (
            mass_dic
            if mass_dic
    ...
    # Clean-up
    df_out["composition"] = [
        glycan_to_composition(k[0][0]) if k else val
        for k, val in zip(df_out["predictions"], df_out["composition"])
    ]
    df_out["charge"] = (
        round(df_out["composition"].apply(composition_to_mass) / df_out.index)
        * multiplier
    )
    ...
    if intensity:
        df_out["rel_abundance"] = (
            df_out["rel_abundance"] / df_out["rel_abundance"].sum() * 100
        )
    else:
